...
- Lee la BBDD vía el MCP configurado en `mcp/database/config.json` (solo `SELECT` / `SHOW` / `EXPLAIN` / `DESCRIBE`).
...
- `$RUBBER_DUCK_HOME/mcp/database/config.example.json` — cómo configurar el MCP.
