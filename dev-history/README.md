...
| `2026-05-28_plan_analyze_hidden_markers_and_plan_file_input_v1.md` | 2026-05-28 | 2 features: (A) marcadores rubber-duck ocultos en Jira (texto ADF blanco `#FFFFFF`; A1 entity-property descartado por falta de tool MCP; verificado en PANA-4171) + aviso de fidelidad de media; (B) `duck-plan <archivo.md>` como input alternativo (file-mode en `bin/duck.sh` con detección soft). S1+S2 completados y cerrados. |
| `2026-05-29_plan_rename_onboarding_to_ask_v1.md` | 2026-05-29 | Rename `duck-onboarding` → `duck-ask` + cambio de responsabilidad: de onboarding de devs nuevos a **experto consultable** de new-admin/old-admin a nivel junior **y** senior, con regla de **honestidad** ("no lo sé", nunca inventar). S1 (rename: command+agent vía `git mv`, `setup.sh`/`uninstall.sh`/`help.sh`) + S2 (prompt experto multinivel) completados. Sin cambios en `bin/duck.sh`. Doc actualizada en `README.md`/`SPEC.md`. |
